...
void main() {
    ...
    float w = minWidth + ((width-minWidth) * float(indexIn) / float(elementCount));
    //float w = width;
    vec3 forward = normalize(normalMatrix * directionIn);//pos.xyz-prev.xyz);
    ...
    alpha = 4 * float(indexIn) / float(elementCount);
}
